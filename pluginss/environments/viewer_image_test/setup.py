...
def GetBeamPos(imname, v_loc):
    ...
    signal= data[12]
    noise= data[13]
    signal_noise_ratio= float(signal)/float(noise)

    return x_centroid, y_centroid, x_peak, y_peak, x_nsig, x_psig, y_nsig, y_psig, signal_noise_ratio

def Dist(p1, p2, p3 = None, p4 = None):
    ...
    return np.sqrt(x_dist), np.sqrt(y_dist), np.sqrt(dist)
